...
export function OrderConfirmation() {
  ...
  const expectedDelivery = deliveryDateLabel(new Date());
  return (
    <main className="mx-auto max-w-[680px] px-6 py-16 text-center">
      <section className="rounded-[8px] border border-[var(--color-border)] bg-[var(--color-surface)] p-8">
        ...
        <p className="mt-1 text-[13px] text-[var(--color-muted)]">Expected delivery: {expectedDelivery}</p>
...
function deliveryDateLabel(fromDate: Date): string {
  const deliveryDate = new Date(fromDate);
  deliveryDate.setDate(fromDate.getDate() + DELIVERY_ESTIMATE_DAYS);
